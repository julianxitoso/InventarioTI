...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_POST['editar_activo_submit'])) {
                ...
                if ($stmt->execute()) {
                    ...
                    if ($datos_anteriores_del_activo) {
                        ...
                        $campos_a_comparar = [
                            ...
                            'valor_aproximado'=>'valor_aproximado', 'detalles'=>'detalles', 'regional_activo_form'=>'regional', // 'regional_activo_form' es el name del input, 'regional' es la columna DB
                            'procesador'=>'procesador', 'ram'=>'ram', 'disco_duro'=>'disco_duro', 
                            'tipo_equipo'=>'tipo_equipo', 'red'=>'red', 'sistema_operativo'=>'sistema_operativo', 
                            'offimatica'=>'offimatica', 'antivirus'=>'antivirus'
                        ...
                        if (!empty($cambios_descripcion_array)) {
                            ...
                            $datos_anteriores_especificos = []; foreach(array_keys($datos_nuevos_para_historial) as $ckey){ $datos_anteriores_especificos[$ckey] = $datos_anteriores_del_activo[$ckey]??($datos_anteriores_del_activo[ucfirst($ckey)]??null);}
        ...
        // La lógica de traslado masivo es AJAX y ya reenvía cedula_original_busqueda y regional_original_busqueda.
        // Se debe añadir empresa_original_busqueda si el JS que arma el FormData lo toma de un campo.
        // El PHP que recibe el AJAX no recarga toda la página, sino que devuelve JSON.
        // La recarga se hace con JS: setTimeout(()=>{ const u=new URL(window.location.href);window.location.href=`editar.php?cedula=${encodeURIComponent(u.searchParams.get('cedula')||'')}&regional=${encodeURIComponent(u.searchParams.get('regional')||'')}`;},1500);
        // Este JS de recarga necesitará tomar el valor del filtro empresa también.

        // ... (contenido de confirmar_traslado_masivo) ...
        // Dentro de este bloque, el registro de historial ya usa $usuario_actual_sistema_para_historial
        // Si la empresa cambia con el traslado, el historial debería reflejarlo.
        // La descripción del historial $desc_t_h podría incluir la empresa origen/destino.
        // Y $datos_o_e_h y $datos_destino_hist también podrían incluir la empresa si es relevante para el historial de traslado.

        // Por ahora, la lógica de POST para traslado no necesita cambios directos para la búsqueda,
        // pero el JS que hace el fetch y la recarga SÍ necesitará considerar el filtro empresa.
        // (Esto se aborda en la sección <script> al final)

        // --- INICIO CÓDIGO ORIGINAL TRASLADO (abreviado para brevedad, sin cambios internos funcionales) ---
        if (!tiene_permiso_para('trasladar_activo')) { /* ... error ... */ exit; }
        header('Content-Type: application/json'); $response = ['success' => false, 'message' => '', 'data_acta' => null];
        // ... (captura de datos POST para traslado) ...
        $cedula_origen_del_traslado = $_POST['cedula_usuario_origen_hidden'] ?? ''; // Este es el que se está editando
        // ... (validaciones) ...
        // Aquí es donde se actualiza la BD
        // $sql_t = "UPDATE activos_tecnologicos SET cedula=?, nombre=?, cargo=?, regional=? WHERE id IN ($ids_sql)";
        // Si la EMPRESA también se traslada (o se establece una específica para el nuevo responsable), se añadiría aquí.
        // Actualmente no se está actualizando la columna 'Empresa' en el traslado.

        // ... (resto de la lógica de traslado y registro de historial) ...
        echo json_encode($response); exit;
        // --- FIN CÓDIGO ORIGINAL TRASLADO ---
...
if ($criterio_buscada_activo) {
    ...
    if (!empty($empresa_buscada)) { // <<< Añadir filtro de empresa
        ...
        $params_select_query[] = $empresa_buscada; $types_select_query .= 's';
    }
        ...
        <?php if ($criterio_buscada_activo): ?>
            ...
            <?php elseif (!empty($activos_encontrados)) :
                ...
                foreach ($activos_encontrados as $activo_map) { // Renombrado $activo a $activo_map para evitar colisión con $activo_individual
                    // Usar 'Empresa' como clave si así es como se devuelve de la BD, basado en el debug de buscar.php
                    $asset_forms_data[$activo_map['cedula']]['info'] = ['nombre' => $activo_map['nombre'], 'cargo' => $activo_map['cargo'], 'cedula' => $activo_map['cedula'], 'empresa_responsable' => $activo_map['Empresa'] ?? ($activo_map['empresa'] ?? '')];
                    $asset_forms_data[$activo_map['cedula']]['activos'][] = $activo_map;
                }
                    ...
                    <div class="user-asset-group mt-4" id="user-group-<?= htmlspecialchars($cedula_grupo) ?>">
                        <div class="user-info-header">
                            ...
                            <div class="actions-block">
                                ...
                                <button type="button" class="btn btn-sm btn-outline-primary btn-transfer-modal" data-bs-toggle="modal" data-bs-target="#trasladoModal" data-cedula-origen="<?= htmlspecialchars($cedula_grupo) ?>" data-empresa-origen="<?= htmlspecialchars($data_grupo['info']['empresa_responsable'] ?? '') ?>">
                    ...
                    <div class="row">
                        <div class="col-md-6 border-end">
                            <p>Del usuario: <strong id="nombreUsuarioOrigenTrasladoModal"></strong><br>
                               ...
                               Empresa Origen: <strong id="empresaUsuarioOrigenTrasladoModal"></strong>
                            </p>
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function () {
            ...
            if (trasladoModalEl) {
                ...
                var empresaOrigenActualParaModal = ''; // <<< Para guardar empresa origen
                ...
                trasladoModalEl.addEventListener('show.bs.modal', function (event) {
                    ...
                    empresaOrigenActualParaModal = button.getAttribute('data-empresa-origen'); // <<< Obtener empresa origen
                    
                    var userGroupDiv = document.getElementById(`user-group-${cedulaOrigenActualParaModal}`);
                    var listaActivosUl = document.getElementById('listaActivosTrasladar'); 
                    listaActivosUl.innerHTML = '';

                    document.getElementById('empresaUsuarioOrigenTrasladoModal').textContent = empresaOrigenActualParaModal || 'N/A'; // <<< Mostrar empresa origen

                    if(userGroupDiv){
                        // ... (código para llenar nombre y cédula origen, y lista de activos)
                        var nombreUsuarioPElement = userGroupDiv.querySelector('.user-info-header h4');
                        var cedulaUsuarioPElement = userGrmodalDarBajaoupDiv.querySelector('.user-info-header h4 small.text-muted');
                    ...
                    document.getElementById('nueva_empresa_traslado').value = ''; // <<< Limpiar campo nueva empresa
                    ...
                    const inputNuevoCargo = document.getElementById('nuevo_cargo_traslado'); 
                    let timeoutId = null;
                    inputNuevaCedula.onkeyup = function () { /* ... (tu código de autocompletar cédula) ... */ };
                });
                ...
                document.getElementById('confirmarTrasladoBtn').addEventListener('click', function() {
                    ...
                    if(!nCed||!nNom||!nCar||!nReg||!nEmp){alert('Datos del nuevo responsable o empresa incompletos.'); return;} // <<< Añadir nEmp a la validación
