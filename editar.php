...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $empresa_buscada = $_POST['empresa_original_busqueda'] ?? ($_GET['empresa'] ?? '');
} else { // GET request
    $cedula_buscada = $_GET['cedula'] ?? '';
    $regional_buscada = $_GET['regional'] ?? '';
    $empresa_buscada = $_GET['empresa'] ?? '';
    if (!empty($cedula_buscada) || !empty($regional_buscada) || !empty($empresa_buscada)) {
        $criterio_buscada_activo = true;
    }
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_POST['editar_activo_submit'])) {
        ...
        // <<< AJUSTE FECHA COMPRA: Añadir 'fecha_compra' a la validación de campos obligatorios
        } elseif (empty($_POST['id']) || empty($_POST['tipo_activo']) || empty($_POST['marca']) || empty($_POST['serie']) || empty($_POST['estado']) || !isset($_POST['valor_aproximado']) || empty($_POST['regional_activo_form']) || empty($_POST['fecha_compra'])) {
            $mensaje = "<div class='alert alert-danger'>Error: Faltan campos obligatorios del activo para actualizar (incluyendo Fecha de Compra).</div>";
        } else {
            ...
            // <<< AJUSTE FECHA COMPRA: Añadir `fecha_compra = ?` a la consulta UPDATE
            $sql = "UPDATE activos_tecnologicos SET tipo_activo=?, marca=?, serie=?, estado=?, valor_aproximado=?, detalles=?, regional=?, procesador=?, ram=?, disco_duro=?, tipo_equipo=?, red=?, sistema_operativo=?, offimatica=?, antivirus=?, fecha_compra=? WHERE id=?";
            $stmt = $conexion->prepare($sql);
            if (!$stmt) {
                $mensaje = "<div class='alert alert-danger'>Error al preparar actualización: " . $conexion->error . "</div>";
            } else {
                // <<< AJUSTE FECHA COMPRA: Añadir 's' para la fecha y la variable $_POST['fecha_compra'] al bind_param
                $stmt->bind_param(
                    'ssssdsssssssssssi', // 15 s/d + 's' para fecha_compra + 'i' para id
                    $_POST['tipo_activo'],
                    $_POST['marca'],
                    $_POST['serie'],
                    $_POST['estado'],
                    ...
                    $_POST['offimatica'],
                    $_POST['antivirus'],
                    $_POST['fecha_compra'], // Nueva variable
                    $id_activo_a_editar
                );
                if ($stmt->execute()) {
                    ...
                    if ($datos_anteriores_del_activo) {
                        ...
                        // <<< AJUSTE FECHA COMPRA: Añadir 'fecha_compra' a los campos a comparar para el historial
                        $campos_a_comparar = [
                            ...
                            'antivirus' => 'antivirus',
                            'fecha_compra' => 'fecha_compra' // Nuevo campo
                        ];
                        foreach ($campos_a_comparar as $post_key => $db_key) {
                            $valor_nuevo = $datos_nuevos_post[$post_key] ?? null;
                            $valor_anterior = $datos_anteriores_del_activo[$db_key] ?? null;
                            // Si el valor nuevo es una cadena vacía y el anterior era NULL, trátalos como iguales para evitar logs innecesarios
                            if ($valor_nuevo === '' && $valor_anterior === null) {
                                // No hacer nada, son equivalentes para campos opcionales
                            } elseif ($valor_nuevo != $valor_anterior) { // Usar != para comparación de tipo flexible con NULL
                        ...
                        if (!empty($cambios_descripcion_array)) {
                            ...
                            foreach (array_keys($datos_nuevos_para_historial) as $ckey) {
                                $datos_anteriores_especificos[$ckey] = $datos_anteriores_del_activo[$ckey] ?? null;
                            }
        ...
        // ... (código de traslado sin cambios) ...
        ...
        $nueva_empresa = $_POST['nueva_empresa_traslado'] ?? '';
        ...
        // ... (código de dar baja sin cambios) ...
...
$opciones_tipo_activo = ['Computador', 'Monitor', 'Impresora', 'Escáner', 'DVR', 'Contadora Billetes', 'Contadora Monedas', 'Celular', 'Impresora Térmica', 'Combo Teclado y Mouse', 'Diadema', 'Hub Multipuertos/Red', 'Telefono', 'Router', 'Otro']; // Añadido 'Otro'
$opciones_tipo_equipo = ['Portátil', 'Mesa', 'Todo en 1']; 
$opciones_red = ['Cableada', 'Inalámbrica', 'Ambas']; 
$opciones_estado_general = ['Bueno', 'Regular', 'Malo', 'En Mantenimiento', 'Nuevo', 'Dado de Baja']; // Asegurar que 'Nuevo' esté y 'Dado de Baja' no se edite a este
...
<body>
                                ...
                                <div class="asset-form-outer-container <?= $clase_enfocada ?>" id="activo_container_<?= htmlspecialchars($form_id_individual_activo) ?>">
                                    ...
                                    <form method="post" action="editar.php" id="form-activo-<?= htmlspecialchars($form_id_individual_activo) ?>">
                                        ...
                                        <div class="row">
                                            ...
                                            // <<< AJUSTE FECHA COMPRA: Mostrar el campo de fecha de compra
                    ...
                    <div class="modal-body">
                        <div class="row">
                            <div class="col-md-6 border-end">
                                <p>Del usuario: <strong id="nombreUsuarioOrigenTrasladoModal"></strong><br>C.C: <strong id="cedulaUsuarioOrigenTrasladoModal"></strong><br>Empresa Origen: <strong id="empresaUsuarioOrigenTrasladoModal"></strong><br>Regional Origen: <strong id="regionalUsuarioOrigenTrasladoModal"></strong></p>
                            ...
                            <div class="col-md-6">
                                ...
                                <div class="mb-2"><label for="nuevo_nombre_traslado" class="form-label form-label-sm">Nuevo Nombre Completo</label><input type="text" class="form-control form-control-sm" id="nuevo_nombre_traslado" required></div>
                                <div class="mb-2"><label for="nuevo_cargo_traslado" class="form-label form-label-sm">Nuevo Cargo</label><input type="text" class="form-control form-control-sm" id="nuevo_cargo_traslado" required></div>
                                <div class="mb-2"><label for="nueva_regional_traslado" class="form-label form-label-sm">Nueva Regional (del activo)</label><select class="form-select form-select-sm" id="nueva_regional_traslado" required><option value="">Seleccione Regional...</option><?php foreach ($regionales as $r): ?><option value="<?= htmlspecialchars($r) ?>"><?= htmlspecialchars($r) ?></option><?php endforeach; ?></select></div>
                                <div class="mb-2"><label for="nueva_empresa_traslado" class="form-label form-label-sm">Nueva Empresa (del activo)</label><select class="form-select form-select-sm" id="nueva_empresa_traslado" required><option value="">Seleccione Empresa...</option><?php foreach ($empresas_disponibles as $e_option): ?><option value="<?= htmlspecialchars($e_option) ?>"><?= htmlspecialchars($e_option) ?></option><?php endforeach; ?></select></div>
                            </div>
                        </div>
                    </div>
    ...
    <script>
        ...
        function mostrarInfoModal(titulo, mensaje) {
            const modalElement = document.getElementById('infoModal'); // Asumiendo que tienes un modal #infoModal en tu HTML base
        ...
        function habilitarCamposActivo(formSuffix) {
            ...
            const firstEditableField = form.querySelector('select[name="tipo_activo"]'); // O el primer campo que quieras enfocar
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            if (trasladoModalEl) {
                ...
                document.getElementById('confirmarTrasladoBtn').addEventListener('click', function() {
                    ...
                    const nEmp = document.getElementById('nueva_empresa_traslado').value;
                    ...
                    fd.append('nueva_empresa_traslado', nEmp);
                    bsTrasladoModal.hide();
                    ...
                    .then(r => r.json()).then(d => {
                        ...
                        if (d.success) {
                            setTimeout(() => {
                                ...
                                const empresaF = encodeURIComponent(document.getElementById('empresa_buscar').value || '');
                                window.location.href = `editar.php?cedula=${cedulaF}&regional=${regionalF}&empresa=${empresaF}`;
                            }, 1500);
                        }
                    }).catch(e => { console.error('Error:', e); mostrarInfoModal('Error','Error procesando traslado.'); });
                });
            }
            <?php endif; ?>
            <?php if (tiene_permiso_para('dar_baja_activo')): ?>
            ...
            if (modalDarBajaEl) {
                modalDarBajaEl.addEventListener('show.bs.modal', function(event) {
                    ...
                    console.log("ID del activo capturado del botón data-id-activo:", idActivo); // DEBUG LINE
                    ...
                    document.getElementById('empresaOriginalBusquedaBajaModal').value = document.getElementById('empresa_buscar').value;
                });
            }
            <?php endif; ?>
