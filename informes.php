...
if (!function_exists('displayStars')) {
    function displayStars($rating, $totalStars = 5) { /* ... tu función displayStars ... */ 
...
// Constante para el tipo de evento de mantenimiento
...
if ($tipo_informe_seleccionado !== 'seleccione') {
    switch ($tipo_informe_seleccionado) {
        case 'general':
            // ... tu código para informe general ...
            $titulo_informe_actual = "Informe General de Activos (Agrupado por Responsable)";
            $query = "SELECT activos_tecnologicos.*, activos_tecnologicos.nombre as nombre_responsable_directo, activos_tecnologicos.cedula as cedula_responsable_directo, activos_tecnologicos.cargo as cargo_responsable_directo, usuarios.aplicaciones_usadas FROM activos_tecnologicos LEFT JOIN usuarios ON activos_tecnologicos.cedula = usuarios.usuario WHERE estado != 'Dado de Baja' ORDER BY Empresa ASC, cedula_responsable_directo ASC, nombre_responsable_directo ASC, id ASC";
            ...
            break;
        case 'por_tipo':
            // ... tu código para informe por tipo ...
            ...
            break;
        case 'por_estado':
            // ... tu código para informe por estado ...
            ...
            break;
        case 'por_regional':
            // ... tu código para informe por regional ...
            ...
            break;
        case 'por_empresa':
            // ... tu código para informe por empresa ...
        ...
        case 'calificacion_por_tipo': 
            // ... tu código para informe de calificaciones ...
            ...
            break;
        case 'dados_baja':
            // ... tu código para informe de dados de baja ...
            ...
            break;
        case 'movimientos':
            // ... tu código para informe de movimientos ...
        ...
        // <<< NUEVO INFORME MANTENIMIENTO: Inicio Case >>>
        case 'activos_con_mantenimientos':
            $titulo_informe_actual = "Informe de Activos con Mantenimientos Realizados";
            // Esta consulta trae cada mantenimiento como una fila, repitiendo los datos del activo.
            // Asegúrate que la constante HISTORIAL_TIPO_MANTENIMIENTO esté definida correctamente.
            ...
            $query = "SELECT 
                        ...
                        at.marca, 
                        at.serie, 
                        at.nombre AS responsable_nombre, 
                        at.cedula AS responsable_cedula, 
                        at.Empresa AS activo_empresa,
                        at.regional AS activo_regional,
                        at.estado AS activo_estado,
                        h.fecha_evento AS fecha_registro_historial, 
                        h.datos_nuevos AS datos_mantenimiento_json
                      FROM 
                        activos_tecnologicos at
                      JOIN 
                        historial_activos h ON at.id = h.id_activo
                      WHERE 
                        h.tipo_evento = '$tipo_mantenimiento_const' 
                        AND at.estado != 'Dado de Baja'
                      ORDER BY 
                        at.id ASC, 
                        STR_TO_DATE(JSON_UNQUOTE(JSON_EXTRACT(h.datos_nuevos, '$.fecha_reparacion')), '%Y-%m-%d') DESC";
            
            $resultado_query = mysqli_query($conexion, $query);
            if ($resultado_query) {
                $datos_para_tabla = mysqli_fetch_all($resultado_query, MYSQLI_ASSOC);
            } else {
                error_log("Error en la consulta (activos_con_mantenimientos): " . mysqli_error($conexion));
            }
            // Estas son las columnas que se mostrarán directamente en la tabla principal.
            // Los detalles del mantenimiento se extraerán del JSON.
            $columnas_tabla_html = [
                "ID Activo", "Tipo", "Marca", "Serie", "Responsable", "Empresa", "Fecha Reparación", "Diagnóstico", "Costo", "Proveedor", "Téc. Interno", "Detalle Reparación"
            ];
            break;
        // <<< NUEVO INFORME MANTENIMIENTO: Fin Case >>>
...
<div class="container-main container">
    ...
    <div class="row mb-5 justify-content-center">
        ...
        <div class="col-lg-3 col-md-4 col-sm-6 mb-3"> 
            <a href="informes.php?tipo_informe=activos_con_mantenimientos" class="card informe-selector-card text-decoration-none <?= ($tipo_informe_seleccionado == 'activos_con_mantenimientos') ? 'border-primary shadow' : 'text-dark' ?>"> 
                <div class="card-body"> 
                    <i class="bi bi-tools text-primary"></i> <h5>Activos con Mantenimientos</h5>
                </div>
            </a>
        </div>
        </div>
    ...
    <?php if ($tipo_informe_seleccionado !== 'seleccione' && !empty($titulo_informe_actual)) : ?>
        ...
        <?php if (!empty($datos_para_tabla)) : ?>
            <?php
            // --- LÓGICA GENERAL PARA MOSTRAR TABLAS ---
            if ($tipo_informe_seleccionado == 'general') {
                // ... tu código existente para informe general ...
                ...
                // ... tu código existente para estos informes ...
            ...
            // <<< NUEVO INFORME MANTENIMIENTO: Inicio Display Tabla >>>
                            ...
                            <tbody>
                                <?php foreach ($datos_para_tabla as $fila) : ?>
                                    <?php 
                                        ...
                                        if ($fecha_reparacion !== 'N/A' && !empty($fecha_reparacion)) {
                                            try {
                                                $fecha_reparacion = date("d/m/Y", strtotime($fecha_reparacion));
                                            } catch (Exception $e) { $fecha_reparacion = $mantenimiento_info['fecha_reparacion'];} // Mantener original si el formato es inesperado
                                        }
                                        $diagnostico = $mantenimiento_info['diagnostico'] ?? 'N/A';
                                        $costo_reparacion = isset($mantenimiento_info['costo_reparacion']) && is_numeric($mantenimiento_info['costo_reparacion']) 
                                                            ? '$' . number_format(floatval($mantenimiento_info['costo_reparacion']), 0, ',', '.') 
                                                            : 'N/A';
                                    ...
                                    ?>
                                    <tr>
                                        <td><?= htmlspecialchars($fila['activo_id']) ?></td>
                                        <td><?= htmlspecialchars($fila['tipo_activo']) ?></td>
                                        <td><?= htmlspecialchars($fila['marca']) ?></td>
                                        <td><?= htmlspecialchars($fila['serie']) ?></td>
                                        <td><?= htmlspecialchars($fila['responsable_nombre'] ?? 'N/A') ?> (<?= htmlspecialchars($fila['responsable_cedula'] ?? 'N/A') ?>)</td>
                                        <td><?= htmlspecialchars($fila['activo_empresa'] ?? 'N/A') ?></td>
            ...
            <?php
            // <<< NUEVO INFORME MANTENIMIENTO: Fin Display Tabla >>>
            } 
            ?>
        <?php else: ?>
