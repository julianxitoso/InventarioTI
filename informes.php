...
if (!isset($conexion) || (method_exists($conexion, 'connect_error') && $conexion->connect_error) || !$conexion) {
    ...
    $conexion_error_msg = "Error crítico de conexión. Contacte al administrador.";
} else {
...
if ($tipo_informe_seleccionado !== 'seleccione' && !$conexion_error_msg) {
    ...
    switch ($tipo_informe_seleccionado) {
        ...
        // --- INICIO DE LA CORRECCIÓN PARA MANTENIMIENTOS ---
        case 'activos_con_mantenimientos':
            $titulo_informe_actual = "Informe de Activos Actualmente en Mantenimiento";
            $query = "SELECT {$campos_base_select} 
                      {$joins_base} 
                      WHERE at.estado = 'En Mantenimiento' {$condiciones_fecha_activo} 
                      ORDER BY at.id ASC";
            // No se necesitan parámetros extra para esta consulta, los de fecha ya se manejan globalmente.
            $columnas_tabla_html = ["ID Activo", "Tipo", "Marca", "Serie", "Estado Actual", "Responsable", "C.C. Resp.", "Empresa", "Fecha Compra"];
            break;
        // --- FIN DE LA CORRECCIÓN PARA MANTENIMIENTOS ---

        // --- INICIO DE LA CORRECCIÓN PARA PRÉSTAMOS ---
        case 'activos_en_prestamo': 
            ...
            break;
        // --- FIN DE LA CORRECCIÓN PARA PRÉSTAMOS ---
...
<head>
    ...
    <style>
        ...
        .informe-selector-card { transition: transform .2s, box-shadow .2s; cursor: pointer; border: 1px solid #ddd; background-color: #fff; color: #212529; text-decoration: none; }
        .informe-selector-card:hover { transform: translateY(-5px); box-shadow: 0 8px 20px rgba(0,0,0,0.12); }
        .informe-selector-card .card-body { display: flex; flex-direction: column; justify-content: center; align-items: center; min-height: 130px; text-align: center; padding: 1rem; }
        .informe-selector-card i { font-size: 2.2rem; margin-bottom: 0.75rem; }
        .informe-selector-card h5.card-title { font-size: 1.1rem; font-weight: 500; margin-bottom: 0; color: #343a40; }
        .informe-selector-card:hover h5.card-title { color: #0d6efd; }
...
<div class="container-main container">
        ...
        <div class="col">
            <a href="#" data-tipo="activos_con_mantenimientos" class="text-decoration-none">
                <div class="card informe-selector-card h-100 <?= ($tipo_informe_seleccionado == 'activos_con_mantenimientos') ? 'border-primary shadow' : '' ?>">
                    <div class="card-body"> <i class="bi bi-tools text-primary"></i> <h5 class="card-title mt-2">Activos en Mantenimiento</h5></div>
                </div>
            </a>
        </div>
        <div class="col"> 
    ...
    <?php if ($tipo_informe_seleccionado !== 'seleccione' && !empty($titulo_informe_actual) && $titulo_informe_actual !== "Tipo de Informe No Válido") : ?>
        ...
        <?php if (!empty($datos_para_tabla)) : ?>
            <?php
            // --- INICIO CORRECCIÓN: Lógica de renderizado de la tabla ---
                                ...
                                <?php foreach ($datos_para_tabla as $item) : ?>
                                    <tr>
                                        ...
                                        <?php elseif ($tipo_informe_seleccionado == 'dados_baja'): ?>
                                            <td><?= htmlspecialchars($item['id'] ?? 'N/A') ?></td>
                                            <td><?= htmlspecialchars($item['nombre_tipo_activo'] ?? 'N/A') ?></td>
                                            <td><?= htmlspecialchars($item['marca'] ?? 'N/A') ?></td>
                                            <td><?= htmlspecialchars($item['serie'] ?? 'N/A') ?></td>
                                            <td><?= htmlspecialchars($item['empresa_responsable'] ?? 'N/A') ?></td>
                                        ...
                                        <?php elseif ($tipo_informe_seleccionado == 'activos_con_mantenimientos'): ?>
                                            ...
                                            <td><?= htmlspecialchars($item['serie'] ?? 'N/A') ?></td>
                                            <td><span class="<?= getEstadoBadgeClass($item['estado']) ?>"><?= htmlspecialchars($item['estado'] ?? 'N/A') ?></span></td>
                                            <td><?= htmlspecialchars($item['nombre_responsable'] ?? 'N/A') ?></td>
                                            <td><?= htmlspecialchars($item['cedula_responsable'] ?? 'N/A') ?></td>
                                            <td><?= htmlspecialchars($item['empresa_responsable'] ?? 'N/A') ?></td>
                                            <td><?= htmlspecialchars(!empty($item['fecha_compra']) ? date("d/m/Y", strtotime($item['fecha_compra'])) : 'N/A') ?></td>
                                        <?php elseif ($tipo_informe_seleccionado == 'activos_en_prestamo'): ?>
                                            ...
                                            <td><?= htmlspecialchars($item['marca'] ?? 'N/A') ?></td>
                                            <td><?= htmlspecialchars($item['serie'] ?? 'N/A') ?></td>
                                            <td><?= htmlspecialchars($item['Codigo_Inv'] ?? 'N/A') ?></td>
