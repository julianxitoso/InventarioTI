...
$pdf->Row([
    $activo_data['Codigo_Inv'],
    $activo_data['serie'],
    $activo_data['marca'],
    $activo_data['nombre_tipo_activo'],
    ($activo_data['estado'] == 'Bueno' ? 'X' : ''),
    ($activo_data['estado'] == 'Regular' ? 'X' : ''),
    ($activo_data['estado'] == 'Malo' ? 'X' : '')
]);

$pdf->MultiCell(195, 20, $pdf->to_iso('OBSERVACIONES GENERALES: '), 1, 'L');
$pdf->Ln(2);
