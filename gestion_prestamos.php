...
$usuarios_para_prestamo = []; 
if (!$conexion_error_msg) { 
    $result_usuarios = $conexion->query("SELECT id, nombre_completo, usuario FROM usuarios WHERE activo = 1 ORDER BY nombre_completo ASC");
    if ($result_usuarios) { while ($row = $result_usuarios->fetch_assoc()) { $usuarios_para_prestamo[] = $row; } }
    else { error_log("Error al cargar usuarios para préstamo: " . $conexion->error); }
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && !$conexion_error_msg) {
    ...
    if (isset($_POST['registrar_prestamo_submit'])) {
        ...
        try {
            ...
            $usuario_receptor_info = array_values(array_filter($usuarios_para_prestamo, function($u) use ($id_usuario_recibe) { return $u['id'] == $id_usuario_recibe; }));
            $nombre_receptor = !empty($usuario_receptor_info) ? $usuario_receptor_info[0]['nombre_completo'] . " (C.C: ".$usuario_receptor_info[0]['usuario'].")" : "ID Usuario: ".$id_usuario_recibe;
...
if (!$conexion_error_msg && !empty($id_usuario_actual_logueado)) { 
    $sql_base_listar_prestamos = "SELECT 
                            ...
                            WHERE (p.id_usuario_presta = ? OR p.id_usuario_recibe = ?)";
    ...
    if ($stmt_listar_prestamos) {
        ...
        if ($stmt_listar_prestamos->execute()) {
            ...
            if ($result_prestamos) { 
                // error_log("[LISTAR_PRESTAMOS] Filas encontradas: " . $result_prestamos->num_rows); // Descomentar para depurar
    ...
    // error_log("[LISTAR_PRESTAMOS] No se listan préstamos porque id_usuario_actual_logueado está vacío."); // Descomentar para depurar
    ...
    // error_log("[LISTAR_PRESTAMOS] No se listan préstamos debido a error de conexión previo."); // Descomentar para depurar
}
                    ...
                    <div id="seccion_detalle_prestamo" style="display:none;"> 
                        ...
                        <div class="mb-3">
                            ...
                            <select class="form-select form-select-sm" id="id_usuario_recibe_prestamo_modal" name="id_usuario_recibe_prestamo" required>
                                <option value="">Seleccione el usuario que recibe...</option>
                                <?php foreach($usuarios_para_prestamo as $usr_opt): ?>
                                    <option value="<?= $usr_opt['id'] ?>"><?= htmlspecialchars($usr_opt['nombre_completo'] . ' (C.C: ' . $usr_opt['usuario'] . ')') ?></option>
                                <?php endforeach; ?>
                            </select>
                        </div>
                        <div class="row">
...
document.addEventListener('DOMContentLoaded', function () {
    // console.log("DOM Cargado. Intentando adjuntar eventos para modales. Rol:", PHP_VARS.currentUserRole, "Cédula:", PHP_VARS.currentUserCedula);
    ...
    if (modalRegistrarPrestamoEl) {
        // console.log("PHP indica abrir modal de creación por error POST.");
        try {
            const modalCrear = bootstrap.Modal.getOrCreateInstance(modalRegistrarPrestamoEl);
            modalCrear.show();
            // console.log("Modal de creación abierto por JS debido a error_creacion_prestamo.");
        } catch (e) {
    ...
    // Definición de la función limpiarCamposSeleccionActivo
    ...
    // Definición de la función mostrarMensajeBusqueda
    ...
    // Función para limpiar el modal de creación completo
    function limpiarModalCreacion() {
        ...
        if(formCrear) {
            // Solo resetear campos que no se prellenan automáticamente o que el usuario debe ingresar
            if(!cedulaPrestadorInput || !cedulaPrestadorInput.readOnly) {
                 // Si es admin, podría resetear todo el form, pero es mejor ser selectivo
                 // formCrear.reset(); // Esto podría borrar la cédula si es admin y ya la ingresó
            }
            // Campos que siempre se deben resetear o poner a valor por defecto
            if(selectActivoModal) selectActivoModal.innerHTML = '<option value="">Seleccione un activo...</option>'; // Limpiar opciones de activos
            if(document.getElementById('id_usuario_recibe_prestamo_modal')) document.getElementById('id_usuario_recibe_prestamo_modal').value = "";
        ...
        limpiarCamposSeleccionActivo(); // Limpiar la parte de selección de activos específicamente
    ...
    if (modalRegistrarPrestamoEl) {
        modalRegistrarPrestamoEl.addEventListener('show.bs.modal', function () {
            ...
            if (PHP_VARS.currentUserRole !== 'admin' && PHP_VARS.currentUserCedula) {
                ...
                if (cedulaPrestadorInput) {
                    cedulaPrestadorInput.readOnly = false;
                }
                 if(seccionBusquedaPrestador) seccionBusquedaPrestador.style.display = 'flex'; 
                if (btnBuscarActivos) {
                    btnBuscarActivos.style.display = 'inline-block'; 
                }
            }
        });
    }
    ...
    if (selectActivoModal && infoEstadoActualEl && hiddenEstadoActualEl && btnSubmitPrestamo) {
        ...
        // Estado inicial del botón de submit y campos de info del activo
        limpiarCamposSeleccionActivo(); 
    }
});
</script>
